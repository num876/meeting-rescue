...
export default function SignupPage() {
  ...
  const handleOAuthSignup = async (provider: AuthProvider) => {
    ...
    try {
      ...
      if (error.message.includes('auth/popup-closed-by-user')) return
      setError(error.message || 'Failed to create account with provider')
    } finally {
      setLoading(false)
    }
  }
      ...
      >
        ...
        <div className="glass-card bg-surface-0/80 p-8 rounded-3xl border border-border/50 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h1 className="text-2xl font-bold tracking-tight text-primary mb-2 text-center">Create an account</h1>
            <p className="text-sm text-secondary text-center mb-8">
              Start extracting meeting insights in seconds.
            </p>
            ...
            <div className="mt-6">
              <button
                ...
                disabled={loading}
                className="btn-secondary w-full py-2.5 hover:bg-surface-2 transition-colors disabled:opacity-50 flex items-center justify-center"
              >
